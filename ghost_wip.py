...
class ManhattanGhost(Agent):
    ...
    def getDistribution(self, state):
        ...
        capsulePosition = matrixAsList(state.getCapsules()) #get list of existing capsule locations
        #locate capsule
        ...
        ghost_to_goal= min(distancesToGoal) #shortest distance to the goal (pacma or capsule) after step is taken
...
class GuardGhost(Agent):
    ...
    def getDistribution(self, state):
        ...
        # Select best actions given the state
        distancesToPacman = [shortest_path(walls, pos, pacmanPosition) for pos in newPositions] #finds the distance from ghost to pacman after tking the next step
        if self.index>len(capsulePosition): #checks how many ghosts are left
            distancesToGoal=distancesToPacman #makes pacman the ghosts goal
            pac_to_cap=1 #treat it as if pacman is close to capsule
        else:
            distancesToGoal = [shortest_path(walls, pos, capsulePosition[self.index-1]) for pos in newPositions] #makes capsule the ghosts goal
            pac_to_cap= shortest_path(walls, capsulePosition[self.index-1], pacmanPosition)

        ghost_to_goal= min(distancesToGoal) #shortest distance to the goal (pacma or capsule) after step is taken

        d = shortest_path(walls, pos, pacmanPosition) #current distanc to pacman
